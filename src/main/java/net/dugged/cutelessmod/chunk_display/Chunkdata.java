...
// Implementation of chunk debug tool taken almost completly from
// https://github.com/X-com/CarpetClient/blob/master/src/main/java/carpetclient/gui/chunkgrid/
// Thanks to Xcom, 0x, Skyrising and all others that helped create it
// Implementation tries to use native Forge networking
...
public class Chunkdata implements Serializable {
	...
	private static final Comparator<FullEvent> spacialSorted = new Comparator<FullEvent>() {
		@Override
	...
	private static final Comparator<FullEvent> temporalSorted = new Comparator<FullEvent>() {
		@Override
		public int compare(FullEvent e1, FullEvent e2) {
			if (e1.t != e2.t) {
	...
	private final int clearCount = 0;
	...
	private static void startStopRecording(boolean start) {
		CarpetPluginChannel.CARPET_CHANNEL.sendToServer(new CarpetPacket(CarpetPacket.CHUNK_LOGGER, new PacketBuffer(Unpooled.buffer().writeBoolean(start))));
	}
	...
	public void addData(int gametick, int eventNumber, int x, int z, int d, int eventcode, int traceid, int reason) {
		try {
			...
			FullEvent event = new FullEvent(x, z, d, eventcode, traceid, gametick, eventNumber, reason);
	...
	private static class FullEvent {
		...
		public String toString() {
			return String.format("X: %d Z: %d D: %d t: %d O: %d event: %s stack: %d reason: %s", x, z, d, t, o, e.toString(), s, r);
		}
	}
	...
	public class MapView implements Iterable<ChunkView> {
		...
		private final int clearCount = Chunkdata.this.clearCount;
