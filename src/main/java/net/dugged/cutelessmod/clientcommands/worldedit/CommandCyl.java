...
public class CommandCyl extends CommandBase {
	@Override
		...
		if (args.length >= 3 && args.length <= 4) {
			if (WorldEdit.hasSelection() && WorldEdit.widthX() == 1 && WorldEdit.widthY() == 1 && WorldEdit.widthZ() == 1) {
				...
				while (x <= z) {
					...
					pos2 = new BlockPos(WorldEdit.posA.getX() - x, pos.getY() + height - 1, WorldEdit.posA.getZ() - z);
					handler.fill(pos, pos2, blockstate);
					pos = new BlockPos(WorldEdit.posA.getX() + x, WorldEdit.posA.getY(), WorldEdit.posA.getZ() - z);
					pos2 = new BlockPos(WorldEdit.posA.getX() - x, pos.getY() + height - 1, WorldEdit.posA.getZ() + z);
					handler.fill(pos, pos2, blockstate);
					pos = new BlockPos(WorldEdit.posA.getX() + z, WorldEdit.posA.getY(), WorldEdit.posA.getZ() + x);
					pos2 = new BlockPos(WorldEdit.posA.getX() - z, pos.getY() + height - 1, WorldEdit.posA.getZ() - x);
					handler.fill(pos, pos2, blockstate);
					pos = new BlockPos(WorldEdit.posA.getX() + z, WorldEdit.posA.getY(), WorldEdit.posA.getZ() - x);
					pos2 = new BlockPos(WorldEdit.posA.getX() - z, pos.getY() + height - 1, WorldEdit.posA.getZ() + x);
					handler.fill(pos, pos2, blockstate);
