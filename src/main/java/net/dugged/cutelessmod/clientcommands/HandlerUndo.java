		...
		if (!started) {
			...
			if (message) {
				mc.ingameGUI.getChatGUI().printChatMessage(new TextComponentTranslation(
					"text.cutelessmod.clientcommands.undo.startedSaving"));
			}
		}
		int threshold = BLOCKS_PROCESSED_PER_TICK / ClientCommandHandler.instance.countHandlerType(
			HandlerUndo.class);
		int iterCount = 0;
		...
		if (iterators.isEmpty()) {
			...
			if (message) {
				mc.ingameGUI.getChatGUI().printChatMessage(
					new TextComponentTranslation("text.cutelessmod.clientcommands.undo.savedUndo"));
			}
			finished = true;
		}
	}
